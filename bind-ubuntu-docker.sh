...
then
   ...
   then 	   
      ...
      docker rm $imagename
      docker container run --name=$imagename --mount type=bind,source=/container-data/bind-container/,target=/shared-mount/  --mount type=bind,source=/rotating-backups,target=/backups  -p 5300:53 -p 5300:53/udp -d -i -t  digitalreachinsight/bind-ubuntu:$version
   fi
fi
