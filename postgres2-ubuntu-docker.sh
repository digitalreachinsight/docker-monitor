...
then
   ...
   then 	   
      ...
      docker rm $imagename
      docker container run --name=$imagename --mount type=bind,source=/container-data/postgres-container/,target=/shared-mount/  --mount type=bind,source=/rotating-backups,target=/backups  -p 172.17.0.1:5432:5432  -d -i -t digitalreachinsight/postgres2-ubuntu:$version
   fi
fi
