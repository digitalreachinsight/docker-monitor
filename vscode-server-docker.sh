...
then
   ...
   then
      ...
      docker rm $imagename
      docker container run --hostname=$1 --name=$imagename -e PUID=33 -e PGID=33 -e PASSWORD=vscode2023! --mount type=bind,source=/container-data/,target=/container-data/  -p 8443:8443 -d -i -t  lscr.io/linuxserver/code-server:$version
   fi
fi
